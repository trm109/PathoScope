...
def processPathoMap(pathoMapOptions):
    ...
    if (
        len(procPathoMapOptions.inReadFilePair1) > 0
        and len(procPathoMapOptions.inReadFilePair2) > 0
        and len(procPathoMapOptions.inReadFile) > 0
    ):
    ...
    for indexPrefix in procPathoMapOptions.targetIndexPrefixes:
        bowtie2Options.btIndexPrefix = (
            procPathoMapOptions.indexDir + os.sep + indexPrefix
        )
    ...
    if len(procPathoMapOptions.targetAlignFiles) > 1:
        appendAlignFile = (
            procPathoMapOptions.outDir
            + os.sep
            + procPathoMapOptions.exp_tag
            + "appendAlign.sam"
        )
        if pathoMapOptions.verbose:
