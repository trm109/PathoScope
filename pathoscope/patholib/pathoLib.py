...
def append_ti_into_fasta_app(
    ...
    GET_ALL_TAX = -2
    NAs = "X"
    (hostname, port, user, passwd, defaultDb) = list(range(5))
    useHash = False
    ...
    if (
        MySqlConf[passwd] == NAs
    ):  # then, we use download gi2tax instead of using mysql query
        useHash = True
    ...
    if not useHash:
        con = dbUtils.init_mysql_innocentive(
            MySqlConf, 0
        )  # DON'T REMOVE LAST 0 or CHANGE TO 1
    ...
    if taxon_ids[0] == GET_ALL_TAX:
        ...
        if subTaxF:
            ...
            subTaxons = get_allsub_taxons_phylo(
                h_tax_node2par, taxon_ids, exclude_taxon_ids
            )

    # append taxonomy id to fasta header
    if useHash:
        append_ti_into_fasta_hash(
            ncbiNt,
            gi2taxDump,
            subTaxons,
            enable_descF,
...
def gi2taxid_offline(mxGi, gi2taxDump, rerunF):
    ...
    if rerunF or (not os.path.isfile(file2sv)):
        ...
        gi2Taxid = []
        gi2Taxid = [0 for x in range(mxGi + 1)]
        for line in fp:
            entry = line.split("\t")
            ti = int(entry[1])
...
def taxon_rel_in_hash(nodes_fn):
    ...
    NOT_VISIT, INCLUDE, EXCLUDE = list(range(3))
    tax = [-1, 0]
    h_tax_node2par = {}
    for n in fp:
...
def append_ti_into_fasta_mysql(
    con, nt, Ti2sel, enable_descF, enable_onlineF, nt2, noTaxIdFa, invalSelFlag
):
                ...
                if tiReadyF:
                    ...
                    if ti == NOT_VALID:
                        ...
                        if get_all_taxF or (ti in Ti2sel):
                            ...
                            organismName = re.sub(r"\s+", "_", organismName)
                            if enable_descF and r.description:
                                fp2.write(
                                    ">ti|%d|org|%s|%s\n%s\n"
                                    % (ti, organismName, r.description, r.seq)
                                )
                            else:
                                fp2.write(
                                    ">ti|%d|org|%s|%s\n%s\n"
...
def append_ti_into_fasta_hash(
    nt, gi2taxFn, Ti2sel, enable_descF, enable_onlineF, nt2, noTaxIdFa, invalSelFlag
):
            ...
            if tiReadyF:
                ...
                for r in seqParse.parse(fp, "fasta"):
                    ...
                    if gi > maxGi or gi2ti[gi] == NOT_AVAIL:
                        if enable_onlineF:
                            genbank_id = mObj.group(
                                2
                            )  # telling exactly, it must be any gene name in a database
                            # genbank_id=entries[3] #telling exactly, it must be any gene name in a database
                            ti = pathoUtilsA.ncbi_eutil(
                                gi, genbank_id, TAXONOMY_ID
                            )  # updated ti
                        else:
                            ti = NOT_VALID
                    else:
...
def build_innocentive_hg19_tgt_db(ncbiNt, ncbiGi2TaxConvFn, ncbiCatFn, nodes_fn, outD):
    ...
    # 1. read gi2ti.dump
    ##############################
    (ncbiNt_ti, ncbiNt_ti_invalid) = append_ti_into_fasta_app(
        ncbiNt, [-2], ncbiGi2TaxConvFn, nodes_fn, 1
    )
...
def get_genebank_file_gi(gi, gb_report):
    cmd = (
        'curl -s "http://eutils.ncbi.nlm.nih.gov/entrez/eutils/efetch.fcgi?db=nucleotide&id=%s&rettype=gb&retmode=xml" -o %s'
        % (gi, gb_report)
    )
    os.system(cmd)
    gotIt = True
...
# =================================
def csv_update_delim(
    fp, gi, sub_gi, gene, locus_tag, protein_id, strand, stbp, edbp, pkey
):
    fp.write(
        "%d\t%d\t%d\t%s\t%s\t%s\t%s\t%d\t%d\n"
...
def getGi2TaxDump_online(downloadD):
    ...
    gi2taxDump = pathoUtilsA.wget_download2(
        gi2taxFtp, downFbase, downExt, downloadD, "select", "gi_taxid_nucl.dmp"
    )
    return gi2taxDump
...
def getNodesDump_online(downloadD):
    ...
    nodesDfn = pathoUtilsA.wget_download2(
        phyloNodeFtp, downFbase, downExt, downloadD, "select", "nodes.dmp"
    )
    return nodesDfn
...
def parse_ncbi_entry(entry):
    gi, ref_name, taxonId, organism, lineage, product, stbp, edbp = (
        "0",
        "",
        "0",
        "",
        ...
        "0",
        "0",
    )  ## set all values of interest equal to an empty string
    sub_gis = []
    E = len(entry)
    for x in enumerate(entry):
        ...
        if tmp[:10] == "ORGANISM  ":
            if not organism:
                ...
                while (
                    (j < E)
                    and (entry[j].strip()[:9] != "REFERENCE")
                    and (entry[j].strip()[:8] != "FEATURES")
                ):
                    if j > (x[0] + 1):
                        lineage += " "
        ...
        if tmp[:6] == "CDS   ":
            ...
            gi_sub, gene_sub, locus_tag_sub, product_sub, protein_id_sub = (
                "0",
                "",
                "",
                "",
...
def gb2prepare_load_data_file(MySqlConf, tiNtDfn, downloadD):
    ...
    dummy = pathoUtilsA.wget_download2(
        gbFlatFtp, downFbase, downExt, downloadD_gff, "nothing", "X"
    )
    ...
    dummy = pathoUtilsA.wget_download2(
        refSeqFlatFtp, downFbase, downExt, downloadD_gff, "nothing", "X"
    )
    ...
    GI, REF_NAME, TAXON_ID, ORGANISM, LINEAGE, PRODUCT, STBP, EDBP, SUB_GI = list(
        range(9)
    )
    (
        GI_SUB,
    ...
    ##################################################################$
    ANNO_T, DELIM_T, TAX_T = list(range(3))
    ...
    for gbFlatFn in os.listdir(downloadD_gff):
        ...
        if (
            gbFlatFn.endswith(gbDnExt[0])
            or gbFlatFn.endswith(gbDnExt[1])
            or gbFlatFn.endswith(gbDnExt[2])
        ):  # or gbFlatFn!='gbcon208.seq.gz': #debug
        ...
        for x in fp:
            if re.search(
                ...
                pkey_anno, fps[ANNO_T] = csv_update_anno_gi(
                    fps[ANNO_T],
                    gB[GI],
                    gB[REF_NAME],
                    gB[EDBP],
                ...
                for s in gB[SUB_GI]:
                    ...
                    pkey_delim, fps[DELIM_T] = csv_update_delim(
                        fps[DELIM_T],
                        gB[GI],
                        s[GI_SUB],
                        s[GENE_SUB],
                ...
                if h_taxLookup.get(gB[TAXON_ID], -1) == -1:
                    pkey_ti, fps[TAX_T] = csv_update_ti(
                        fps[TAX_T], gB[TAXON_ID], gB[ORGANISM], gB[LINEAGE], pkey_ti
                    )
...
def update_taxT_fn(h_ti_dbSz, taxT_fn):
    ...
    for i in fp:
        ...
        fp2.write(
            "%s\t%s\t%d\t%s\t%s"
            % (ent[0], ent[1], h_ti_dbSz.get(ent[1], 1), ent[2], ent[3])
        )
    fp.close()
    fp2.close()
