...
def pathoscope_reassign(pathoIdOptions):
    ...
    (initPi, pi, _, NU) = pathoscope_em(
        U, NU, genomes, maxIter, emEpsilon, verbose, piPrior, thetaPrior
    )
...
def pathoscope_em(U, NU, genomes, maxIter, emEpsilon, verbose, piPrior, thetaPrior):
    ...
    ### Initial values
    pi = [1.0 / G for _ in genomes]
    initPi = pi
    theta = [1.0 / G for _ in genomes]
    ...
    for i in range(maxIter):  ## EM iterations--change to convergence
        ...
        for j in NU:  # for each non-uniq read, j
            ...
            xtmp = [
                1.0 * pitmp[k] * thetatmp[k] * z[1][k] for k in range(len(ind))
            ]  ### Calculate unormalized xs
            xsum = sum(xtmp)
            if xsum == 0:
                xnorm = [0.0 for k in xtmp]  ### Avoiding dividing by 0 at all times
            else:
                xnorm = [1.0 * k / xsum for k in xtmp]  ### Normalize new xs
        ...
        # M step
        pisum = [
            thetasum[k] + pisum0[k] for k in range(len(thetasum))
        ...
        if totaldiv == 0:
            totaldiv = 1
        pi = [
            (1.0 * k + pip) / (Utotal + NUtotal + pip * len(pisum)) for k in pisum
        ]  ## update pi
        if i == 0:
            initPi = pi
        ...
        if NUtotaldiv == 0:
            NUtotaldiv = 1
        theta = [
            (1.0 * k + thetap) / (NUtotaldiv + thetap * len(thetasum)) for k in thetasum
        ]
