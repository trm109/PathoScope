...
parser.add_argument(
    "-e", action="store", default="testset", dest="exp_tag", help="Experiment tag"
)
parser.add_argument(
    "-outdir", action="store", default=".", dest="outdir", help="Output Directory"
)
...
parser.add_argument(
    "-f", action="store", dest="ali_file", required=True, help="Alignment file path"
)

inputArgs = parser.parse_args()
start = time()
PathoID.pathoscope_reassign(
    inputArgs.out_matrix,
    inputArgs.verbose,
    inputArgs.score_cutoff,
    inputArgs.exp_tag,
    inputArgs.ali_format,
    inputArgs.ali_file,
    inputArgs.outdir,
    inputArgs.emEpsilon,
    inputArgs.maxIter,
    not (inputArgs.noalign),
)
elapsed = time() - start
