...
subparsers = parser.add_subparsers(
    dest="subcommand", help="Select one of the following sub-commands"
)

# create the parser for the "LIB" command
parser_a = subparsers.add_parser(
    "LIB", help="Pathoscope taxon level reference genome Library creation Module"
)
...
parser_a.add_argument(
    ...
    const=True,
    default=False,
    help="Do not keep an additional description in original fasta seq header."
...
parser_a.add_argument(
    ...
    const=True,
    default=False,
    help="To include all sub taxonomies under the query taxonomy id."
...
parser_a.add_argument(
    "--online",
    action="store_const",
    dest="lib_online_search",
    required=False,
    const=True,
    default=False,
    help="To enable online searching in case you cannot find a "
...
parser_a.add_argument(
    ...
    required=False,
    default="localhost",
    help="specify hostname running mysql if you want to use mysql "
...
parser_a.add_argument(
    ...
    default=3306,
    type=int,
    help="provide mysql server port if different from default (3306)",
)
parser_a.add_argument(
    "-dbuser",
    action="store",
...
parser_c.add_argument(
    ...
    default=False,
    dest="id_nocutoff",
    help="Do not cutoff display of genomes, even if it is insignificant",
)
parser_c.add_argument(
    "-scoreCutoff",
    action="store",
...
parser_d.add_argument(
    "-outDir", action="store", default=".", dest="rep_outdir", help="Output Directory"
)
parser_d.add_argument(
    "--contig",
    action="store_true",
    default=False,
    dest="rep_contig_flag",
...
parser_e = subparsers.add_parser(
    "QC", help="PathoQC: Quality control program for high throughput sequencing reads"
)
parser_e.add_argument(
    ...
    required=True,
    default="",
    help="1st pair of read in PER or SER",
)
parser_e.add_argument(
    "-2",
    action="store",
...
parser_e.add_argument(
    ...
    default=0,
    type=int,
    help="let us know a mean read length (0:ignore. [1]:I want to know the range of read length after trimming, i:user_specified_mean_read_length)",
)
parser_e.add_argument(
    "-t",
    action="store",
    dest="qc_phred_offset",
    required=False,
    default=33,
    type=int,
    help="specify a phred offset used in encoding base quality(0:not sure?, [33]:phred+33, 64:phred+64)",
)
parser_e.add_argument(
    "-o",
    action="store",
    ...
    required=False,
    default="",
    help="specify output directory in full path",
)
parser_e.add_argument(
    "-s",
    action="store",
    ...
    required=False,
    default="Illumina",
    help="tell us which sequencer generates the reads ([Illumina], PacBio, Roche454, IonTorrent)",
)
parser_e.add_argument(
    "-m",
    action="store",
    ...
    type=int,
    default=35,
    help="specify the min read length cutoff[35]",
)
parser_e.add_argument(
    "-a",
    action="store",
    ...
    required=False,
    default="N",
    help="specify an adaptor (Y:have pathoQC detect it, [N]:ignore, ACGT:adaptor)",
)
parser_e.add_argument(
    "-a2",
    action="store",
    ...
    required=False,
    default="N",
    help="specify a second adaptor if it is different from the first one (Y:have pathoQC detect it, [N]:ignore, ACGT:adaptor)",
)
...
parser_e.add_argument(
    ...
    type=int,
    default=0,
    help="specify a cutoff of base quality value to trim at the end of reads([0]:ignore, 1:let pathoQC take care of it, i:user_specified_cutoff_value)",
)
parser_e.add_argument(
    "-R",
    action="store",
    ...
    type=int,
    default=0,
    help="set to 1 if you want to mask all lower case bases that may correspond to sequence tag or adaptor in Roche454 or IonTorrent",
)
parser_e.add_argument(
    "-e",
    action="store",
    dest="qc_coff_entropy",
    required=False,
    type=int,
    default=30,
    help="specify a cutoff of entropy of low sequence complexity to filter out[0..100],default:30, set 0 to disable",
)
parser_e.add_argument(
    "-d",
    action="store",
    ...
    type=int,
    default=1,
    help="filtering duplicates: [1] (exact duplicate), 2 (5' duplicate), 3 (3' duplicate), 4 (reverse complement exact duplicate), 5 (reverse complement 5'/3' duplicate)",
)
parser_e.add_argument(
    "-g",
    action="store",
    dest="qc_add_valid_single",
    required=False,
    type=int,
    default=0,
    help="Set to 1 if you want to add a valid single read into a reduced valid PER set. Note that this option works only with PER",
)
parser_e.add_argument(
    "--no_prinseq",
    action="store_const",
    ...
    const=False,
    default=True,
    help="to force to skip prinseq module",
)
...
parser_e.add_argument(
    ...
    type=int,
    default=1,
    help="specify a total number of cpus to use[1]",
)
parser_e.add_argument(
    "--debug",
    action="store_const",
...
def main():
    ...
    if inputArgs.subcommand == "LIB":
        ...
        MysqlConf = (
            inputArgs.lib_dbhost,
            inputArgs.lib_dbport,
            inputArgs.lib_dbuser,
            inputArgs.lib_dbpasswd,
            inputArgs.lib_db,
        )
    ...
    if inputArgs.subcommand == "REP":
        ...
        mysqlConf = (
            inputArgs.rep_dbhost,
            inputArgs.rep_dbport,
            inputArgs.rep_dbuser,
            inputArgs.rep_dbpasswd,
            inputArgs.rep_db,
        )
    ...
    if inputArgs.subcommand == "QC":
        qcargs = sys.argv[2:]
        pathoqcdir = (
            pathoscopedir + os.path.sep + "pathoscope" + os.path.sep + "pathoqc"
        )
