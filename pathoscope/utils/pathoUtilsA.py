...
def ncbi_eutil(gi, genbank_id, entry2sel):
    ...
    for q in queries:
        curlCmd = (
            "curl -s \"http://eutils.ncbi.nlm.nih.gov/entrez/eutils/efetch.fcgi?db=nuccore&id=%s&rettype=fasta&retmode=xml\" | grep %s | cut -d '>' -f 2 | cut -d '<' -f 1 | tr -d \"\\n\""
            % (q, field2sel)
        )
...
def ex_wget_download(protozoaD):
    ...
    if not os.path.exists(protozoaDfn):
        cmd = (
            "cd %s; wget ftp://ftp.ncbi.nlm.nih.gov/refseq/release/protozoa/protozoa.*.rna.fna.gz\n"
            % protozoaD
        )
        cmd = (
            "%swget ftp://ftp.ncbi.nlm.nih.gov/refseq/release/protozoa/protozoa.*.genomic.fna.gz\n"
            % cmd
        )
        cmd = "%sgunzip %s/*.gz\n" % (cmd, protozoaD)
...
def search_cat_in_online_taxonomy(ti, ncbiLineage, tmpFn):
    ...
    field2sel = "Lineage"
    curlCmd = (
        "curl -s \"http://eutils.ncbi.nlm.nih.gov/entrez/eutils/efetch.fcgi?db=taxonomy&id=%d&rettype=fasta&retmode=xml\" | grep -iP '%s' > %s"
        % (ti, field2sel, tmpFn)
    )
    os.system(curlCmd)
