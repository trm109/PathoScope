...
def processPathoReport(pathoReportOptions):
    ...
    if pathoReportOptions.samFile is not None:
        ...
        (bestHitFinalReads, bestHitFinal, level1Final, level2Final) = computeBestHit(
            U, NU, genomes, reads
        )
        ...
        header = [
            ...
            "MapQ Guess",
            "MapQ Best Hit",
            "MapQ Best Hit Read Numbers",
            "MapQ High Confidence Hits",
            "MapQ Low Confidence Hits",
            "Alignment Guess",
            "Alignment Best Hit",
            "Alignment Best Hit Read Numbers",
            "Alignment High Confidence Hits",
            "Alignment Low Confidence Hits",
        ]
        nR = len(reads)
        nG = len(h_refRead)
        (_, _, _, _, _, _, _, _, _, _, _) = write_tsv_report(
            outTsv,
            nR,
            nG,
            pi,
        ...
        if pathoReportOptions.contigFlag:
            bamFile = samUtils.sam2bam(
                pathoReportOptions.samFile, pathoReportOptions.samtoolsHome
            )
            # 3)for each genome, get covered fragment and also make sure that each fragment has a high quality
            refConsFq = samUtils.samtools_consensus(
                bamFile, pathoReportOptions.samtoolsHome
            )
            # 3.1) get delimiter and send a query to mysql to retrieve annotation
            (h_annoT, h_ti_contig) = get_genome_annotation_in_mysql(
                refConsFq,
                pathoReportOptions.minContigLen,
                pathoReportOptions.mysqlConf,
                h_annoT,
                h_ti_contig,
            )
            xmlReport.writePathoXML(
                run_param_str,
                outXml,
                h_annoT,
                h_ti_contig,
                ...
                reads,
                h_readSequence,
                pathoReportOptions.samFile,
                pathoReportOptions.mysqlConf,
            )
        else:
            h_annoT = simple_genome_annotation(
                h_gisPerTi, pathoReportOptions.mysqlConf, h_annoT
            )
            xmlReport.writePathoXML(
                run_param_str,
                outXml,
...
def get_genome_annotation_in_mysql(
    ...
    for r in seqParse.parse(fp, "fastq"):  # for each covered genome
        ...
        covRange = selectConsensusContigs(
            r, minContigLen, -1
        )  # disable checking seq complexity of contig
        ...
        if con:
            mysql_sel_cmd = (
                "select sub_gi, gene, locus_tag, protein_id, stbp, edbp from giDelimT where gi = %s"
                % gi
            )
            cur = con.cursor()
            ...
            if entr:
                ...
                for (
                    j
                ) in entr:  # select which subgi sits within the covered genomic regions
                    aStbp = int(j[STBP])
                    aEdbp = int(j[EDBP])
                    ...
                    for i in range(C):
                        ...
                        notCoveredA -= pathoUtilsA.segments_intersect(
                            aStbp, aEdbp, covRange[i][START], covRange[i][END]
                        )
                    ...
                    if reportA:
                        selCmd = (
                            "select ref_name, product from giAnnoT where gi = %s"
                            % j[SUBGI]
                        )
                        cur = con.cursor()
                        cur.execute(selCmd)
            ...
            for gi in gis:
                ...
                if entr:
                    for (
                        ...
                        entr
                    ):  # select which subgi sits within the covered genomic regions
                        selCmd = (
                            "select ref_name, product from giAnnoT where gi = %s"
                            % j[SUBGI]
                        )
