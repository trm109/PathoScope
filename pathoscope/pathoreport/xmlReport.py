...
def writePathoXML(
    ...
    options.text = (
        samFile + " " + run_parameters
    )  # TODO, split this long string to sub fields
    hostTaxon = ""
    organismsElement = buildOrganismsElement(
        h_annoT,
        h_ti_contig,
        hostTaxon,
        h_refRead,
...
def buildOrganismsElement(
    ...
    for ti in h_gisPerTi:
        ...
        for c in ctgs:
            ...
            contig2.ref_name = c[
                0
            ]  # make sure that all string format only available in xml
    ...
    organismsObj.organisms = sorted(
        organismsObj.organisms, key=lambda x: x.relativeAmount.value, reverse=True
    )
