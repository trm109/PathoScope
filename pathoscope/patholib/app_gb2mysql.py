...
parser.add_argument(
    ...
    dest="ti_nt",
    required=True,
    help="specify a fasta format reference appended with taxonomy id in the front seq header. Refer to app_build_nt_tgt.py to build such fasta file.",
)
parser.add_argument(
    "-d",
    action="store",
    dest="downloadD",
    required=True,
    help="specify a temporary download directory",
)
parser.add_argument(
    "-m",
    action="store",
    ...
    required=False,
    default="localhost",
    help="specify hostname running mysql",
)
parser.add_argument(
    "-P",
    action="store",
